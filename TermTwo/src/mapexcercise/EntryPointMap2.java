...
public class EntryPointMap2 {
	...
	public EntryPointMap2() throws FileNotFoundException {
	...
	private void loadFile() throws FileNotFoundException
	{
		...
		while (sc.hasNextLine()) {
			...
			if(!studentsByName.containsKey(name))   // checks for existence of student by name   //??? if student is has 
			{				
				students.put(unSID++, name);           // add student to map students
				// add elements to map by name as key
				
				Student s = new Student(k, students.get(k)); //add student objects to map with the key student name
			...
			{
				courses.put(course, new Course(course, course));
			}
